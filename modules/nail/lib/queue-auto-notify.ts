...
export async function sendJoinQueueSms(
  ...
  const sms = await sendNailQueueSms(guestPhone, body);
  if (!sms.sent) return;
...
export async function autoNotifyQueueAfterAdvance(
  ...
  if (front?.guest_phone?.trim() && !front.next_sms_sent_at) {
    ...
    const sms = await sendNailQueueSms(front.guest_phone, body);
    if (sms.sent) {
      const now = new Date().toISOString();
      await supabase
        .from("nail_queue")
  ...
  if (second?.guest_phone?.trim() && !second.almost_next_sms_sent_at) {
    ...
    const sms = await sendNailQueueSms(second.guest_phone, body);
    if (sms.sent) {
      await supabase
        .from("nail_queue")
