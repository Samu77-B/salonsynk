...
export function PlatformSubscribeButtons({
  ...
  if (variant === "banner") {
    return (
      <div className="rounded-xl border border-border bg-surface/80 px-4 py-3 sm:px-5 sm:py-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2 shrink-0">
            ...
            {yearlyConfigured && yearlyHref ? (
              <a
                href={yearlyHref}
                className="inline-flex rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-foreground/5"
              >
                Yearly · {yearlyLabel}
              </a>
            ) : null}
        ...
        {(!monthlyConfigured || !yearlyConfigured) && (
          <p className="mt-2 text-xs text-amber-200/90">
            {!monthlyConfigured && !yearlyConfigured
              ? "Subscription prices are not configured yet. Contact support."
              : !yearlyConfigured
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        {monthlyConfigured && monthlyHref ? (
          <a
            href={monthlyHref}
            className="inline-flex justify-center rounded-lg bg-accent px-5 py-2.5 text-sm font-medium text-background"
          >
        ...
        {yearlyConfigured && yearlyHref ? (
          <a
            href={yearlyHref}
            className="inline-flex justify-center rounded-lg border border-border px-5 py-2.5 text-sm font-medium text-foreground hover:bg-foreground/5"
          >
