...
import {
  ...
  updateNailCategory,
  deleteNailCategory,
} from "@modules/nail/actions/services";
...
function CategoryManager({
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setAdding(true);
    setError("");
    const res = await addNailCategory(salonId, { name: n });
    setAdding(false);
    if (res.error) {
      setError(res.error);
    } else {
  ...
  async function handleRename(catId: string) {
    ...
    setSaving(true);
    setError("");
    const res = await updateNailCategory(salonId, catId, { name: n });
    setSaving(false);
    if (res.error) {
      setError(res.error);
    } else {
  ...
  async function handleColorChange(catId: string, hex: string) {
    ...
    setError("");
    const res = await updateNailCategory(salonId, catId, { color: hex });
  ...
  async function handleDelete(cat: CategoryRow) {
    ...
    setError("");
    const res = await deleteNailCategory(salonId, cat.id);
    setDeletingId(null);
    if (res.error) {
      setError(res.error);
    } else {
  ...
  return (
    <div className="rounded-xl border border-border bg-background/60 p-4 shadow-sm sm:p-5 space-y-4">
      ...
      <p className="text-sm text-muted">
        Group services under headings like &ldquo;Gel Manicures&rdquo; or &ldquo;Nail Art&rdquo;. Each category gets a{" "}
...
function ServiceCard({
  ...
  async function save() {
    ...
    const result = await updateNailService(salonId, service.id, {
      name: n,
  ...
  async function remove() {
    ...
    setDeleting(true);
    setFeedback(null);
    const result = await deleteNailService(salonId, service.id);
    setDeleting(false);
    if (result.error) {
      setFeedback("error");
      setFeedbackText(result.error);
    ...
    >
      ...
      <div className="rounded-lg border border-border bg-background/40 p-3">
        <label className="flex cursor-pointer items-start gap-2 text-sm">
          ...
          />
          <span>
            <span className="font-medium text-foreground">Technician can see another client during processing</span>
            <span className="mt-1 block text-muted">
              Use for treatments with drying or setting time: while one client is processing, the diary allows another booking in that
...
export function NailServicesView({
  salonId,
  canManageServices,
  services = [],
  categories = [],
        ...
        <form
          onSubmit={async (e) => {
            ...
            try {
              ...
              const result = await addNailService(salonId, {
          ...
          <div className="rounded-lg border border-border bg-background/40 p-3">
            <label className="flex cursor-pointer items-start gap-2 text-sm">
              ...
              <span>
                <span className="font-medium text-foreground">Technician can see another client during processing</span>
                <span className="mt-1 block text-muted">
                  e.g. gel curing or soak-off — set how long the client is left processing (up to the full duration).
                </span>
              </span>
            </label>
            {newAllowOverlap ? (
