...
import { phoneHref, queueSmsBody } from "@modules/nail/lib/queue-sms-messages";
import {
  startService,
  completeService,
  removeFromQueue,
...
function useRealtimeQueue(salonId: string, serverQueue: QueueEntry[]) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`nail_queue:${salonId}`)
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "nail_queue", filter: `salon_id=eq.${salonId}` },
        (payload) => applyInsert(payload.new as QueueEntry)
      )
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "nail_queue", filter: `salon_id=eq.${salonId}` },
        (payload) => applyUpdate(payload.new as QueueEntry)
      )
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "public", table: "nail_queue", filter: `salon_id=eq.${salonId}` },
...
function AddWalkInForm({
  ...
  return (
    <form action={handleSubmit} className="flex flex-wrap items-end gap-3 rounded-lg border border-border bg-surface p-4">
      ...
      <div className="min-w-[140px]">
        <label htmlFor="preferred_technician_id" className="block text-xs text-muted mb-1">Preferred Technician</label>
        <select
...
function InChairCard({
  ...
  const [isPending, startTransition] = useTransition();
  const [actionError, setActionError] = useState<string | null>(null);
  const technician = members.find((m) => m.id === entry.assigned_technician_id);
  const service = services.find((s) => s.id === entry.service_id);
    ...
    <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3 space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted">
            {technician
              ? `${technician.display_name ?? "Technician"}${technician.station_number ? ` · Station ${technician.station_number}` : ""}`
              : "—"}
