...
type Props = {
  ...
  /** When true, show only "Next available" — no individual technician tiles. */
...
export function JoinQueueForm({
        ...
        {result.smsQueued && (
          <p className="text-sm text-muted">
            {result.position === 1
              ...
              : result.position === 2
                ? `Check your phone — we&apos;ve texted you that you&apos;re #2 in the queue, with around ${AVG_SERVICE_MINUTES} minutes until your turn.`
  ...
  const estimatedWaitMins = queueLength * AVG_SERVICE_MINUTES;
      ...
      <form action={handleSubmit} className="rounded-xl border border-border bg-surface p-5 space-y-4">
        ...
        {(nextAvailableOnly || technicians.length > 0) && (
          <fieldset>
            ...
            {nextAvailableOnly ? (
              <>
                <input type="hidden" name="preferred_technician_id" value="" />
                <div className="flex flex-col items-center gap-2 rounded-xl border border-accent bg-accent/10 ring-2 ring-accent p-4 text-center">
                  ...
                  <span className="text-sm font-medium">Next available technician</span>
              ...
              <>
                <input type="hidden" name="preferred_technician_id" value={preferredTechnicianId} />
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  <button
                    type="button"
                    onClick={() => setPreferredTechnicianId("")}
                  ...
                  {technicians.map((t) => (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => setPreferredTechnicianId(t.id)}
                      className={`flex flex-col items-center gap-2 rounded-xl border p-3 text-center transition-colors ${
                ...
                <p className="text-xs text-muted mt-2">
                  Pick someone you know, or tap their photo. Otherwise we&apos;ll assign the next available technician.
                </p>
              </>
            )}
          </fieldset>
