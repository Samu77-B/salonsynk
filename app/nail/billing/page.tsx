...
export default async function NailBillingPage({
  ...
  if (!tenantRequiresPayment(salonRow)) {
    redirect("/nail/queue");
  }

  const token = (salonRow.payment_invite_token as string | null)?.trim();
  const payUrl = token ? paymentInviteUrl("nail", token) : null;
  const isOwner = (context.member.role ?? "").toLowerCase() === "owner";
  const active = tenantSubscriptionIsActive(salonRow.subscription_status as string | null);

  if (active) {
    redirect("/nail/queue");
  }

  const planPrice = formatPlatformPrice("nail");

  return (
    <div className="mx-auto max-w-lg py-8">
      <h1 className="text-2xl font-bold mb-2">Complete your subscription</h1>
      <p className="text-muted text-sm mb-6">
        Welcome to {platformProductName("nail")} for{" "}
        <strong className="text-foreground">{salonRow.name as string}</strong>. Pay for your first
      ...
      <div className="rounded-xl border border-border p-4 mb-6 space-y-2">
        ...
        <p className="text-lg font-semibold">NailSynk — {planPrice}</p>
        <p className="text-sm text-muted">Live queue, diary, and client tools for your nail salon.</p>
      </div>
      ...
      {isOwner && payUrl ? (
        ...
        <p className="text-sm text-red-400">
          ...
          <a href={`mailto:${NAIL_SITE.email}`} className="underline">
            {NAIL_SITE.email}
          </a>
          .
        </p>
        ...
        <p className="text-sm text-muted">
          Only the salon owner can complete payment. Ask your manager to pay from their welcome email.
        </p>
      )}

      <p className="mt-6 text-xs text-muted">
        Already paid?{" "}
        <Link href="/nail/billing" className="text-accent hover:underline">
          Refresh this page
        </Link>
        .
      </p>
